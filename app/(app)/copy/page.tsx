...
export default async function CopyPage() {
  ...
  return (
    <div>
      <SectionHead
        eyebrow="Copy"
        title="Angles and tracking"
        description="Variants per ICP across email, LinkedIn, voice. Hooks, frameworks, and live performance."
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <Stat
          n={angles.filter((a: any) => a.status === "winning").length}
          label="Winning"
        />
        <Stat
          n={angles.filter((a: any) => a.status === "testing").length}
          label="In test"
        />
      </div>
      ...
      {Object.keys(byICP).length === 0 && (
        <div className="card">
          <div className="empty-state">
            <div className="empty-state-title">No copy angles yet</div>
            <div className="empty-state-hint">
              Seed <code className="kbd">copy_angles</code> to populate this view.
            </div>
          </div>
        </div>
      )}
      ...
      {Object.entries(byICP).map(([icp, rows]) => (
        <section key={icp} className="mb-8">
            ...
            <table className="data">
              <thead>
                <tr>
                  ...
                  <th className="text-right">Mtgs</th>
                  <th>Status</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((a: any) => {
                  ...
                  return (
                    <tr key={a.id}>
                      ...
                      <td className="font-num text-muted text-[11px]">
                        {a.channel}
                      </td>
                      <td>
                      ...
                      <td className="max-w-sm">
                        <div className="text-[12px] font-medium text-ink2">
                          {a.variant_label}
                        </div>
                        <div className="text-[12px] italic text-accent mt-0.5">
                          {a.hook}
                        </div>
                      </td>
                      ...
                      <td>
                        <span className="pill-cell bg-surface2 text-muted">
                          {a.status}
                        </span>
                      </td>
                    </tr>
